...
export function loadContacts() {
  return dispatch => {
    ...
    axios.get('/api/contacts.json').then(
      response => { dispatch(receiveContacts(response.data)) }
    ).catch(
      () => { dispatch(receiveContacts([])) } // FIXME: show an error message
    )
  }
}
...
export function requestContacts() {
  return {
    type: Contact.REQUEST_CONTACTS,
  }
}

export function receiveContacts(contacts) {
  return {
    type: Contact.RECEIVE_CONTACTS,
    contacts,
  }
}
...
mock.onGet('/api/contacts.json').reply(200, [
